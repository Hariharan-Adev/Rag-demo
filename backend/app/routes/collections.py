...
def create_collection(payload: CollectionCreate, request: Request, current_user: dict[str, object] = Depends(get_current_user)):
    ...
    with get_connection() as connection:
        existing = connection.execute(
            "SELECT * FROM document_collections WHERE owner_id = ? AND name = ? COLLATE NOCASE",
            (owner_id, payload.name),
        ).fetchone()
        if existing is not None:
            return dict(existing)
        cursor = connection.execute(
            "INSERT INTO document_collections (owner_id, name) VALUES (?, ?)",
            (owner_id, payload.name),
        )
    ...
    with get_connection() as connection:
        rows = connection.execute(
            """SELECT c.*, COUNT(d.id) AS document_count FROM document_collections c
               LEFT JOIN documents d ON d.collection_id = c.id AND d.owner_id = c.owner_id
...
def get_collection(collection_id: int, current_user: dict[str, object] = Depends(get_current_user)):
    ...
    with get_connection() as connection:
        collection = connection.execute(
            "SELECT * FROM document_collections WHERE id = ? AND owner_id = ?", (collection_id, owner_id)
        ).fetchone()
        if collection is None:
            raise HTTPException(status_code=404, detail="Collection was not found.")
        documents = connection.execute(
            "SELECT id, display_filename AS filename, relative_path, uploaded_at FROM documents WHERE collection_id = ? AND owner_id = ? ORDER BY relative_path, display_filename",
...
def delete_collection(collection_id: int, request: Request, current_user: dict[str, object] = Depends(get_current_user)):
    ...
    with get_connection() as connection:
        exists = connection.execute(
            "SELECT id FROM document_collections WHERE id = ? AND owner_id = ?", (collection_id, owner_id)
        ).fetchone()
        if exists is None:
            raise HTTPException(status_code=404, detail="Collection was not found.")
        connection.execute("BEGIN IMMEDIATE")
        connection.execute(
            "UPDATE documents SET collection_id = NULL, upload_batch_id = NULL, relative_path = NULL WHERE collection_id = ? AND owner_id = ?",
            (collection_id, owner_id),
        )
        connection.execute("DELETE FROM document_collections WHERE id = ? AND owner_id = ?", (collection_id, owner_id))
...
def create_batch(payload: BatchCreate, request: Request, current_user: dict[str, object] = Depends(get_current_user)):
    ...
    with get_connection() as connection:
        collection = connection.execute(
            "SELECT id FROM document_collections WHERE id = ? AND owner_id = ?", (payload.collection_id, owner_id)
        ).fetchone()
        if collection is None:
            raise HTTPException(status_code=404, detail="Collection was not found.")
        cursor = connection.execute(
            """INSERT INTO upload_batches
               (owner_id, collection_id, original_folder_name, total_files, total_bytes)
               VALUES (?, ?, ?, ?, ?)""",
            (owner_id, payload.collection_id, payload.original_folder_name, payload.total_files, payload.total_bytes),
        )
...
def get_batch(batch_id: int, current_user: dict[str, object] = Depends(get_current_user)):
    with get_connection() as connection:
        row = connection.execute(
            "SELECT * FROM upload_batches WHERE id = ? AND owner_id = ?", (batch_id, current_user["id"])
        ).fetchone()
    if row is None:
...
def skip_batch_files(batch_id: int, payload: BatchSkip, current_user: dict[str, object] = Depends(get_current_user)):
    ...
    with get_connection() as connection:
        row = connection.execute(
            "SELECT id FROM upload_batches WHERE id = ? AND owner_id = ?", (batch_id, owner_id)
        ).fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="Upload batch was not found.")
...
def cancel_batch(batch_id: int, request: Request, current_user: dict[str, object] = Depends(get_current_user)):
    ...
    with get_connection() as connection:
        cursor = connection.execute(
            "UPDATE upload_batches SET status = 'cancelled', completed_at = CURRENT_TIMESTAMP WHERE id = ? AND owner_id = ? AND status NOT IN ('completed', 'partially_completed', 'failed')",
            (batch_id, owner_id),
        )
